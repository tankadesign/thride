...
**Product north star (re-steered post-M2): Thride is the realtime look-dev and procedural-material tool for the Three.js ecosystem — a Substance-like app for the web.** Bring finished geometry from Blender/C4D as editable **quad/n-gon meshes** (not glTF-triangulated soup — glTF triangulation breaks editing and animating), build node-driven **procedural materials** that are WYSIWYG-identical to final output in WebGPU/TSL, then **bake** them to PBR image maps for performance **while the procedural source stays live and revisable in the file**. Ship out two ways: standard **glTF + baked PBR** for any Three.js app, or a live **`.thride`** rendered by an **embeddable Thride viewer** that keeps the material procedural. Blender 5's aesthetic remains the visual bar; scope excludes character rigging, hair, and simulation.

**On the pivot:** the app was built (M0–M2) as a capable general DCC in the spirit of C4D/Blender and an "open-source Spline" (direct-manipulation, 3D pen tool, slider extrude/bevel — all shipped). That foundation stays, but **modeling is now a _supporting_ capability, not the pitch — it is frozen** (no new modeling chunks; knife/loop-cut remain 1.x). The differentiators going forward are the procedural material system plus four pillars: **a node-based ("Substance-lite") material editor, n-gon import, bake-while-keeping-source, and the embeddable viewer.** (The M2 layer stack proved the material stack; the node editor is its go-forward form.) See the re-ordered Milestones (M3→M11) below; the prior M3–M6 ordering is superseded. **The interactive UV editor is dropped** — UVs are bring-your-own, projection-based, or headless auto-unwrap.
...
2. **UI:** Hybrid — React DOM shell (docking, menus, inspectors) + canvas-rendered specialized editors (timeline, curve editor). Not a from-scratch canvas UI toolkit. (The node-based material editor is built on **Rete.js** (`rete` + React render plugin) — DOM-based, not canvas — with fully custom themed components, and lives in `ui/`.)
...
| UI shell                | React 19 + dockview (docking) + **Tailwind CSS 4 + daisyUI 5** (sunset theme, smallest size variants, megamenu menus; reach for a daisyUI component before a bare element)                                                                                              |
| UI state binding        | Plain-TS document core + event bus, bridged into React via **jotai** atoms in `src/ui/hooks` (nested domain dirs; multi-hook files named by domain, e.g. `history.ts`). Render layer reads editor state via the `EditorViewportState` contract in `src/types/editor.ts` |
...
src/
  ...
  editors/      # 2D CANVAS UI: timeline/ curves/ (shared canvas kit in editors/kit/) — uv/ dropped
...
- **Milestone gates (every milestone): hard stops.** When a milestone's chunks are done: write the progress file, run the full test suite + the milestone's manual QA checklist, review the work (code + UX), and wait for user sign-off before starting the next milestone. Never continue past a milestone in the same breath.
...
- **Events:** fine-grained typed emitter (`scene:node-changed`, `mesh:dirty`, `material:param`, `selection:changed`, …) with per-slice version counters. React panels subscribe via jotai slice-version atoms (`ui/hooks/doc/document.ts`) so a 60 Hz gizmo drag re-renders only panels watching that slice; render systems subscribe directly to the event bus. Ephemeral editor/UI state (viewport layout, pane cameras, palette) lives in jotai atoms under `ui/hooks/editor/` — never in the document.
...
- **Procedural layer system (M2, shipped — the first-pass differentiator):** a material channel (baseColor/roughness/metalness/normal-height/displacement/emissive/alpha) holds a **layer stack**; each layer = source (noise: perlin/fbm/ridged/turbulence/voronoi F1/F2/F2−F1/cells — Maxon-noise-like set — or gradient ramp/checker/brick/image/solid) + remap (gradient ramp, contrast, invert, clamp) + per-layer projection + **blend mode** (normal/multiply/screen/overlay/add/subtract/difference/darken/lighten) + opacity + optional layer mask. The layer stack is **pure serializable data** (`ProceduralMaterialDoc`); a `MaterialCompiler` walks it and emits one TSL graph per channel wired into `MeshStandard/PhysicalNodeMaterial` slots (`colorNode`, `roughnessNode`, `normalNode` via screen-space `bumpMap(height)` — uniform across noise types, composes with baked normal maps; `positionNode` for displacement). **This is superseded going forward by the node-based editor below** (E7) — the layer stack was the M2 proof; the node graph is the go-forward authoring model. Both compile to the same TSL slots, so the render/bake/viewer pipelines are unaffected.
...
- **Node-based material editor (E7 — the go-forward procedural model, "Substance-lite"):** a Blender/Substance-style node graph where noises, textures, and constants pipe through **modifier** nodes (math, ramp, mix, bump) into an **Output** node's channel sockets (color, roughness, metalness, emission, alpha, normal). This is not a replacement for layering — **the graph _is_ the layering**: `mix` nodes (with the same blend-mode set as the layer stack) compose sources exactly as stacked layers do, and `math`/`ramp`/`bump` are the modifiers that make a material editor powerful. TSL is already a node system, so editor wires map 1:1 onto TSL composition; the same compiler slots (`colorNode`/`roughnessNode`/…) are emitted, so render, bake, and the embeddable viewer are unchanged.
  - **Data model:** `MaterialGraphDTO { nodes[], edges[], output, view? }` embedded as an optional `graph?` on `MaterialDTO` — 1:1 lifecycle with the material, so Create/Update/Delete commands, `structuredClone` snapshots, `material:changed` events, serialization, and **undo all work unchanged**. Max one edge per input socket; unknown node kinds survive round-trip (compiler skips → socket default). Implicit socket conversions (one shared matrix): float→vecN splat, vec3↔color identity, vec3→float = Rec.709 luminance, vec2↔vec3 pad/truncate.
  - **v1 node set:** `output`; the 7 noise nodes auto-generated from the `NOISE_DEFS` registry (E2); `coords` (positionLocal/uv), `value`, `color`, `math` (add/sub/mul/div/pow/min/max/abs/clamp), `mix` (blend modes), `ramp` (stops → 256×1 `DataTexture`, texel-updated in place), `texture` (asset by id + offset/repeat/rotation uniforms), `bump` (height + strength → view-space normal via a custom screen-space-derivative TSL `Fn`, since built-in `bumpMap()` only perturbs UV-driven texture heights). Deferred: displacement, normal-map-image, separate/combine XYZ, triplanar/projection nodes, node groups.
  - **Compiler (`materials/graph/`):** `compileGraph(graph, deps) → { channels, uniforms: Map<"nodeId/param", UniformNode>, ramps, structuralKey, errors }` — memoized DFS from Output only (dead nodes never emit); cycles → error + socket fallback, never throw into the render loop; `deps.getTexture(id)` injected by the render layer. `applyGraphParams` writes uniform `.value`s + ramp texels — **the scrub path, zero recompile**. `structuralKey` (node kinds + structural params + edges; excludes positions/view/uniform params) gates recompile: moving nodes and scrubbing never recompile; rewiring does, via `compileAsync` warm-swap.
  - **Render integration:** `render/scene-sync/graphMaterials.ts` caches per-material `{compiled, structuralKey}`; same key → `applyGraphParams` only, changed key → recompile + assign/clear `*Node` slots + `needsUpdate`; a driven alpha channel sets `transparent`/`depthWrite`. No graph → clear slots so the flat params take over. Wired through `MaterialSync` (no `SceneSynchronizer` change — `material:changed` already routes there).
  - **Editor UI (`ui/nodegraph/`):** built on **Rete.js (`rete`)** with its React render plugin (`rete-react-plugin`) plus the area/connection plugins — Rete owns the graph engine, pan/zoom area, node dragging, wire rendering, minimap, and connection interactions; we supply **fully custom, themed node and connection components** (daisyUI `card-xs` bodies with existing widgets — `NumberDrag`, color inputs, ramp stop editor — and typed, color-coded sockets) so it matches Thride's dark look rather than shipping Rete's default styling, plus connection-validity checks (socket convertibility + cycle pre-check). Lives in `src/ui/` (DOM rule — Rete renders DOM/SVG, not a canvas editor). Undo reuses the MaterialEditor preview→commit pattern (scrubs coalesce; each structural action = one labeled command); Rete's engine/graph events map to `graphEdit.ts` DTO helpers. Per-node preview thumbnails clone `noiseThumbnails.ts` (offscreen renderer, disposed on panel close).
...
### UV handling — no interactive UV editor (dropped 2026-07-18)

- **The interactive UV canvas editor is dropped.** A serviceable in-browser UV editor is a large build for a mediocre result — a laborious canvas editor is worse than the alternatives for our audience. UVs come from three places instead: **(1) bring your own** — imported meshes keep their authored `vt`/`heUV` (N1/H3); **(2) projections** — the per-layer/per-channel projection system (UV/flat/triplanar/cylindrical/spherical/camera) textures un-unwrapped geometry directly in-shader, no UVs required; **(3) headless auto-unwrap** — xatlas-web (WASM) in a worker (N2) writes `heUV` islands for imports lacking UVs, needed so bake always has a target. No canvas, no manual island editing.
...
- **A5 Icon set** _(A2, parallel with everything)_ — **hugeicons** wrapped as `Icon<Concept>` components behind the one `src/icons` barrel (global size, one stroke weight): primitives, tools, modes, node kinds. **Superseded:** originally "original SVG icons", but M0X replaced the hand-drawn SVGs with "hugeicons @ global 20px" — this line now describes the shipped decision, not a scope cut. Done when: icon components cover the v1 tool/primitive list, nothing imports `@hugeicons/*` outside the barrel, and no unicode/emoji glyphs stand in for icons. **Panel tab glyphs: dropped** (user call — text tabs).
...
- **B4 React bindings** _(B1, A3)_ — jotai slice-version atoms + domain hooks under `ui/hooks/` (nested dirs, multi-hook files). Done when: a test panel re-renders only on its slice's version bump. **(Done — migrated to jotai post-M0.)**
...
- **C6 Post-FX stack** _(C1, E1)_ — TSL PostProcessing pipeline, per-pane stack: bloom, vignette, chromatic aberration, tonemap, grouped with ambient shadows + reflections under a Post Processing tab in View Settings. **Done:** effects toggle/tune live (uniforms, no graph rebuild) and serialize via `paneDisplays`; the output tail is shared by both SSR graph modes (`DitherOutput.composeOutput`). Effect ORDER is fixed by the tone-map boundary rather than user-sortable (bloom must precede it, lens effects must follow) — revisit if custom shaders land. **User custom shader effects** (TSL/GLSL snippet + declared uniforms, animatable) **deferred to 1.x** (user decision, PROGRESS_24): a large sub-feature — user code compiled at runtime, declared uniforms, and failure handling that must not brick the viewport.
...
- **E3 Layer-stack compiler** _(E1, E2)_ — **Done (M2).** ProceduralMaterialDoc → TSL per channel, blend modes, gradient ramp DataTextures, UniformTable, compileAsync swap: param edits never recompile; structural edits swap hitch-free. **Superseded as the authoring model by E7** (node editor); the compiler infra (uniform table, ramp textures, recompile boundary) is reused.
- **E4 Projections** _(E3)_ — uv/flat/cubic-triplanar/cylindrical/spherical per layer **and per image channel**. **Done** (M2): the projection math ships and each mapping was verified visually on a sphere + cube (no reference-render harness exists — same gap as E2's goldens). Two carve-outs, both recorded rather than silently dropped: **(a) the viewport projection gizmo (texture mode) is deferred to M10 polish** — the ToolRail's Texture-mode button stays disabled until then. Note projections are now the _primary_ UV alternative (the interactive UV editor is dropped), so this gizmo is load-bearing and a candidate to pull earlier; **(b) `camera` projection is out of the UI** until scene cameras exist (F4) — projecting from the _viewing_ camera reads as broken. Its union member + compiler case remain so persisted docs still render.
...
- **E7 Node material editor** _(E1, E2, E3)_ — the "Substance-lite" go-forward procedural authoring model (see Materials § above). `MaterialGraphDTO` as optional `graph?` on `MaterialDTO`; `materials/graph/` compiler (memoized DFS, uniform-scrub path, structuralKey recompile gate, screen-space-derivative bump `Fn`); `render/scene-sync/graphMaterials.ts` + `MaterialSync` integration; **Rete.js (`rete` + React render plugin)** editor in `ui/nodegraph/` with custom themed node/connection components (add-node menu, ramp stop editor) reusing the MaterialEditor undo pattern; per-node offscreen thumbnails. **Staged:** (1) schema + compiler core (output/7 noises/coords/value/color/math/mix) + tests; (2) texture/ramp/bump emitters + seed-from-flat-bindings; (3) live viewport integration (recompile-count tests); (4) panel read-only render + pan/zoom/select; (5) full editing (connect/disconnect/delete/param widgets, undo labels); (6) thumbnails + error badges + optional compileAsync warm-swap. Done when: build a `noise → ramp → color` and a `fractal → bump → normal` graph entirely in the UI, viewport updates live with zero recompile on param scrubs, each structural action is one undo step, and the graph round-trips through save/reload.
...
- **F4 Cameras (+light polish)** _(B1, C1)_ — camera objects with gizmos/frustum helpers, "look through selected"; light viewport gizmos/helpers. (Base lights — spot/point/directional/ambient/hemisphere/area with shadows + targets — landed in M0X.) Done when: any camera bindable to any pane; light/camera helpers render + serialize.
...
- **H3 GLTF import** _(B1, D1)_ — GLB/GLTF → editable document, source preserved in `assets/sources/`. Triangulated import is the default (always works); **N3** adds the opt-in untriangulate toggle. Done when: Khronos sample-asset suite imports without errors.
...
- **H5 Cloud storage** _(H1)_ — **DROPPED for now** (re-steer): S3/GCS deferred indefinitely; local file + OPFS (H2) only.
- **H6 Asset browser** _(H1, A3)_ — project assets panel, drag-drop import (OBJ/GLB/HDR/EXR/images). Done when: drop-a-file-onto-viewport imports and undoes.

**N. Mesh import & look-dev delivery** (re-steer chunks; src: io, geometry, materials, viewer)

- **N1 OBJ import** _(D1)_ — parse OBJ `f v/vt/vn`, weld by position index, carry `vt` as per-corner `faceUVs` into `HEMesh.fromPolygons`. Open meshes import fine (`heTwin=-1`). On non-manifold/bad-winding throw (HEMesh.ts:85,90), fall back to a badged non-editable triangulated display mesh. Raw bytes preserved in `assets/sources/`. Done when: a quad-heavy Blender/C4D OBJ imports as an editable quad mesh; a non-manifold file degrades to the badge.
- **N3 Untriangulate toggle** _(H3, D1)_ — best-effort quad recovery on GLTF import: weld-by-position → triangle adjacency → coplanar/convex/seam-&-material-aware max-weight tri-pairing (Blender "Tris→Quads") → `number[][]` into `HEMesh.fromPolygons`. Topology-_recovery_, NOT a remesher (Instant Meshes/QuadriFlow resample vertices → rejected: breaks bake fidelity + round-trip). v1 = greedy pairing + angle threshold, no iterative optimization. Opt-in UI with a quad-% readout; triangulated is always the fallback. Done when: a GLTF exported from a quad model recovers quad-dominant topology; tri-soup stays triangulated without error.
- **N2 Headless auto-unwrap** _(D1)_ — xatlas-web (WASM) in a worker, writes islands into `heUV`. Fully headless — there is no interactive UV editor (dropped), and this is the only UV-generation path in the app; it exists so bake always has a target. Only needed for imports lacking `vt`. Done when: an un-UV'd import gets non-overlapping islands with no user interaction.
- **B-SEL Baked/procedural selector** _(E6)_ — per-channel "use baked / use procedural" flag persisted in `MaterialDTO` (`bakedActive?` + baked asset ids beside `textures`; must be in the DTO, not editor UI state — the VW viewer shares `proceduralBind.ts`). "Use baked" = skip binding the stack for that channel so the plain baked `.map` prop takes over; `procedural` stays populated so the source is revisable. Done when: a channel shows its baked map in-viewport while its procedural stack persists + re-activates, surviving save/reload.
- **VW `.thride` viewer/runtime** _(H1)_ — read-only build target reusing SceneSynchronizer + MaterialSync + LightSync + SplineSync + `proceduralBind.ts` + `compile.ts` + UniformTable, minus history/selection/gizmos/dockview. Requires first making SceneSynchronizer's editor imports (SelectionOutline, camera helpers, HELPER_LAYER, overlays) optional. Done when: a `.thride` with a live procedural material renders identically in viewer and editor.
- **VH Golden-image harness** _(E6)_ — Playwright + pixelmatch on WebGPU; folded into E6 since "baked matches procedural within tolerance" is inherently a pixel diff. Closes the project's standing gap (no rendered-output regression guard; two shader bugs shipped past tsc+screenshots). Done when: bake-vs-procedural diff + the noise/blend/projection goldens run in CI.

**I. UV editor — DROPPED (2026-07-18)**

- **I1 UV canvas editor / I2 Unwrap ops — dropped, never built.** Reason (owner call): a serviceable in-browser UV canvas is a large build for a mediocre result. Replaced by bring-your-own UVs (import), the projection system, and headless auto-unwrap (N2 — which absorbed I2's xatlas path). See "UV handling" above. The `src/editors/uv/` dir is not created.
...
- **M0X – Expanded foundation (user-directed, in progress):** per-viewport axis indicator, gizmo polish (50% size, primary hover, shift local-snap w/ configurable grid), MMB pane maximize, hugeicons @ global 20px setting, primitive **adjustment handles** (C4D yellow squares, priority over gizmo), **Convert to Mesh** (registry-backed editable kernel meshes), int-stepped params, double-sided default, object **nesting + DnD** with drop indicator, right-click context menu, ⌘G group-under-null, deep collapse/expand; **lights moved up from M3**: all six three.js light types with shadow defaults, Create → Lights submenu, target system (lights/cameras aim at any object; Infinite lights auto-create a target null), viewport background context menu (per-pane camera + display settings), unique sibling naming, option-drag copy in the object manager. More items expected before M1 starts.
...
- **M2 – Materials & look** _(E1–E5, C6, A5; E6 bake excluded):_ material manager, PBR + built-ins, procedural layer stacks with blend modes, projections (per layer + per image channel), noise maps in every map slot, environment dome light + HDR/EXR, reflections (SSR Fast/High + planar), post-FX stack (bloom / chromatic aberration / vignette), color management. **Carve-outs, all user-agreed:** custom shader effects → 1.x; E4's texture-mode projection gizmo → M10 polish; `camera` projection → F4 (needs scene cameras). The M2 procedural **layer stack** is superseded as the authoring model by the M4 **node editor** (E7) — the layer stack was the proof; the node graph is the go-forward.
  **Re-ordered post-M2 (the look-dev re-steer — see Context):** node material editor (E7) lands first to make the material creator compelling, then cloner+cameras/import/bake; file import/export/bake is promoted above animation, animation defers past files, S3/GCS cloud is dropped, and the **interactive UV editor is dropped** (bring-your-own UVs / projections / headless auto-unwrap). New chunks: **E7** node material editor, **N1** OBJ import, **N3** GLTF untriangulate, **N2** headless auto-unwrap, **B-SEL** baked/procedural per-channel selector, **VW** `.thride` viewer, **VH** golden-image harness.

- **M3 – Scatter & cameras — ✅ Done (2026-07-20)** _(F2, F4; + fix Area-light LTC crash):_ cloner + effector, cameras as first-class objects (lights moved into M0X). Done when: 100k instances @60fps, any camera bindable to any pane, area lights don't crash. **Met:** 200k instances run smooth (>100k @60fps, user-verified); per-pane look-through binds any camera to any pane; area-light LTC crash fixed in M0X. Cameras use a C4D/Blender lens model (focal length + X/Y film offset, fixed full-frame sensor) mapped onto three's PerspectiveCamera, with DOF (PBR + look-through only). **Deferred (user-agreed):** orthographic scene cameras; `camera` texture projection (E4, unblocked by F4 but stays deferred → M10); film-offset gizmo/helper base-shift.
- **M4 – Node material editor — ⏳ done-when met, awaiting sign-off (2026-07-20)** _(E7):_ the "Substance-lite" node graph — noises/textures/constants → math/ramp/mix/bump modifier nodes → Output channels — as the go-forward procedural authoring model (supersedes the M2 layer stack; same TSL compile targets, so bake/export/viewer are unaffected). Editor built on **Rete.js (`rete`)** with custom themed components, `graph?` on `MaterialDTO`, uniform-scrub path with zero recompiles. Done when: `noise → ramp → color` and `fractal → bump → normal` graphs build entirely in the UI, viewport is live with no recompile on scrubs, each structural edit is one undo step, and the graph survives save/reload. **(Priority call — placed here to make the material creator compelling early; movable.)** **Met:** shipped in 5 staged commits (`83483bd` schema+compiler → `675046e` editing). A `graph` normalizes with the E3 stack through one `CompiledLook` seam (graph wins), so viewport/bake/thumbnail are unaffected; the compiler's uniform-scrub path is unit-proven (`getGraphCompileCount` unchanged on scrubs); the Rete panel edits (add/connect/delete/param dropdowns) commit one undo step each; right-click Add-Node uses the app context menu; double-click zoom disabled. **Polish shipped too:** Stage 6 (per-node preview thumbnails, error badges, warm-swap), daisyUI-`sunset` node theming, connected-socket fill, option-drag copy, wire re-drag, initial framing — all E7 stages complete.
- **M5 – Import & UV-ready geometry** _(N1, H3, N3, N2):_ **OBJ import** (lossless quads) + **GLTF/GLB import** with an opt-in **untriangulate** toggle (best-effort quad recovery), headless auto-unwrap for un-UV'd imports. Done when: a quad-heavy Blender/C4D OBJ imports editable; GLTF recovers quad-dominant topology with a quad-% readout; un-UV'd import gets `heUV` islands.
- **M6 – Bake & live source** _(E6, B-SEL, VH):_ UV-space bake to WebP with quality preview + height→normal; **per-channel baked/procedural selector** (bake while keeping the procedural source live); **golden-image harness** (the WYSIWYG guard). Done when: a baked map shows in-viewport while its procedural stack persists + re-activates; bake-vs-procedural pixel diff runs in CI.
- **M7 – Files & interchange out** _(H1, H2, H4, H6; H5 dropped):_ `.thride` package (round-trip everything), OPFS autosave/recovery, GLTF export (baked PBR + `EXT_mesh_gpu_instancing`; n-gons triangulated on export), asset browser (drag-drop OBJ/GLB/HDR/images). Done when: round-trip deep-equal golden incl. a frozen v1 sample; gltf-validator clean.
- **M8 – Embeddable viewer** _(VW):_ read-only `.thride` runtime that renders procedural materials live (no editor/gizmos). Done when: a `.thride` with a live procedural material renders identically in viewer and editor.
- **M9 – Animation** _(G1–G3; + promote Sweep generator):_ track model + timeline + curve editor, animatable TRS + generator sliders (extrude/sweep) + procedural material params (on _unbaked_ channels). Done when: keyframe TRS + a generator slider + a procedural param in one e2e.
- **M10 – Polish** _(E4 projection gizmo, A6; UV canvas dropped):_ texture-mode projection gizmo (load-bearing now that projections replace the UV editor — candidate to pull earlier), settings page (incl. undo memory budget), render-to-image, shortcut remapping, layout presets. Done when: settings persist and drive History's budget; projection gizmo manipulates a layer's projection live.
- **M11 – 1.0 hardening** _(J1–J3):_ perf pass (100k+ instances, 1M-tri editing budget), Safari/Firefox fallback QA, crash-recovery drills, demo scenes, docs site.
