...
**On the pivot:** the app was built (M0–M2) as a capable general DCC in the spirit of C4D/Blender and an "open-source Spline" (direct-manipulation, 3D pen tool, slider extrude/bevel — all shipped). That foundation stays, but **modeling is now a _supporting_ capability, not the pitch — it is frozen** (no new modeling chunks; knife/loop-cut remain 1.x). The differentiators going forward are the mature procedural material stack (M2, done) plus three new pillars: **n-gon import, bake-while-keeping-source, and the embeddable viewer.** See the re-ordered Milestones (M3→M10) below; the prior M3–M6 ordering is superseded.
...
- **N2 Headless auto-unwrap** _(D1)_ — xatlas-web (WASM) in a worker, writes islands into `heUV`. Decoupled from the interactive UV editor (I1) so bake isn't gated on the UV canvas. Only needed for imports lacking `vt`. Done when: an un-UV'd import gets non-overlapping islands without opening a UV editor.
...
- **M4 – Import & UV-ready geometry** _(N1, H3, N3, N2):_ **OBJ import** (lossless quads) + **GLTF/GLB import** with an opt-in **untriangulate** toggle (best-effort quad recovery), headless auto-unwrap for un-UV'd imports. Done when: a quad-heavy Blender/C4D OBJ imports editable; GLTF recovers quad-dominant topology with a quad-% readout; un-UV'd import gets `heUV` islands.
...
- **M6 – Files & interchange out** _(H1, H2, H4, H6; H5 dropped):_ `.thride` package (round-trip everything), OPFS autosave/recovery, GLTF export (baked PBR + `EXT_mesh_gpu_instancing`; n-gons triangulated on export), asset browser (drag-drop OBJ/GLB/HDR/images). Done when: round-trip deep-equal golden incl. a frozen v1 sample; gltf-validator clean.
- **M7 – Embeddable viewer** _(VW):_ read-only `.thride` runtime that renders procedural materials live (no editor/gizmos). Done when: a `.thride` with a live procedural material renders identically in viewer and editor.
- **M8 – UV editor & polish** _(I1, E4 gizmo, A6):_ interactive UV canvas, texture-mode projection gizmo, settings page (incl. undo memory budget), render-to-image, shortcut remapping, layout presets. Done when: manual UV edits reflect live and undo.
...
- **M10 – 1.0 hardening** _(J1–J3):_ perf pass (100k+ instances, 1M-tri editing budget), Safari/Firefox fallback QA, crash-recovery drills, demo scenes, docs site.
