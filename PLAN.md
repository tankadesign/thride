...
**On the pivot:** the app was built (M0–M2) as a capable general DCC in the spirit of C4D/Blender and an "open-source Spline" (direct-manipulation, 3D pen tool, slider extrude/bevel — all shipped). That foundation stays, but **modeling is now a *supporting* capability, not the pitch — it is frozen** (no new modeling chunks; knife/loop-cut remain 1.x). The differentiators going forward are the mature procedural material stack (M2, done) plus three new pillars: **n-gon import, bake-while-keeping-source, and the embeddable viewer.** See the re-ordered Milestones (M3→M10) below; the prior M3–M6 ordering is superseded.
...
- **M9 – Animation** _(G1–G3; + promote Sweep generator):_ track model + timeline + curve editor, animatable TRS + generator sliders (extrude/sweep) + procedural material params (on *unbaked* channels). Done when: keyframe TRS + a generator slider + a procedural param in one e2e.
