...
- **M4 – Node material editor** _(E7):_ the "Substance-lite" node graph — noises/textures/constants → math/ramp/mix/bump modifier nodes → Output channels — as the go-forward procedural authoring model (supersedes the M2 layer stack; same TSL compile targets, so bake/export/viewer are unaffected). Editor built on **Rete.js (`rete`)** with custom themed components, `graph?` on `MaterialDTO`, uniform-scrub path with zero recompiles. Done when: `noise → ramp → color` and `fractal → bump → normal` graphs build entirely in the UI, viewport is live with no recompile on scrubs, each structural edit is one undo step, and the graph survives save/reload. **(Priority call — placed here to make the material creator compelling early; movable.)**
